...

;;; the sboxes of DES (with permutation P built in)
...
(defconst des-sbox0 ; S1 + P: 6-to-4
#32@(
...
(defmacro frob (left right shift-amount constant)
...
(defmacro 6-bits (val offset) `(ldb (byte 6 ,offset) ,val))
(defmacro 4-bits (val offset) `(ldb (byte 4 ,offset) ,val))

(defmacro sbox-subst (val sbox0 sbox1 sbox2 sbox3)
...
;; DES round function
(defmacro des-round (left right keys index)
...
(defmacro des-munge-core (left right keys)
  `(do ((round 0 (1+ round))
	(key-index 0 (+ key-index 4)))
       ((>= round (/ *reduced-rounds* 2))) ; old: 8
     ;; TODO: how to support odd rounds?
     (des-round ,left ,right ,keys key-index)
     (des-round ,right ,left ,keys (+ key-index 2))))

(defmacro des-munge-core (left right keys)
  `(do ((round 0 (1+ round))
	(key-index 0 (+ key-index 2)))
       ((>= round *reduced-rounds*))
     (cond ((evenp round)
	    (des-round ,left ,right ,keys key-index))
	   (t
	    (des-round ,right ,left ,keys key-index)))))
...
;; API
(defclass crypto::reduced-des (cipher 8-byte-block-mixin)
...
;; API
(defmethod schedule-key ((cipher crypto::reduced-des) key)
...
(defparameter *reduced-rounds* 16)

;; API
(defcipher crypto::reduced-des
